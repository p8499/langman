...
{	parent.f_setTitle();
	...
	grid_store=new JsonRest({target:"../language/api/role",idProperty:"rlid",sortParam:"orderBy"});
	grid_structure=[
	...
	{	msgAreaAdd_messages.removeAll({group:"add"});
		msgAreaAdd.refresh();
		add_rlid.set("value","");
		add_rlname.set("value","");
		addDialog.show();
	};
	this.f_checkAdd=function()
	{	var deferred=new Deferred();
		xhr.post("../language/api/role_check"+"/"+add_rlid.get("value"),{data:domForm.toJson("addForm"),handleAs:"json",headers:{'Content-Type': 'application/json'}}).then(function(data)
	...
	{	var deferred=new Deferred();
		...
		all([grid.store.get(id),xhr("../language/api/base/lock",{query:{obj:"role",id:id},handleAs:"json"})]).then(function(data)
		{	update_rlid.set("value",data[0].rlid);
			update_rlname.set("value",data[0].rlname);
			updateDialog.show();
			deferred.resolve(data);
		},function(data)
	...
	{	var deferred=new Deferred();
		xhr("../language/api/base/unlock",{query:{obj:"role",id:update_rlid.get("value")},handleAs:"json"}).then(function(data)
		{	updateDialog.hide();
			deferred.resolve(data);
		},function(data)
		{	updateDialog.hide();
	...
	{	var deferred=new Deferred();
		xhr.put("../language/api/role_check"+"/"+update_rlid.get("value"),{data:domForm.toJson("updateForm"),handleAs:"json",headers:{'Content-Type': 'application/json'}}).then(function(data)
		{	deferred.resolve(data);
		},function(data)
	...
	{	var deferred=new Deferred();
		...
		all([grid.store.put(obj),xhr("../language/api/base/unlock",{query:{obj:"role",id:update_rlid.get("value")},handleAs:"json"})]).then(function(data)
		{	updateDialog.hide();
			deferred.resolve(data);
		},function(data)
		{	msgAreaUpdate_messages.removeAll({group:"update"});
	...
	{	select=grid.select.row.getSelected().length;
		...
		{	if(wa&&select==1)
			...
			if((raroleauthority||riroleauthority)&&select==1)
				roleauthorityLink.setAttribute('disabled',false);
			else
				roleauthorityLink.setAttribute('disabled',true);
		}
	...
	{	this.ra=this.wa=false;
		...
		this.raroleauthority=this.riroleauthority=false;
		...
		var draroleauthority=new Deferred(),driroleauthority=new Deferred();
		var fra=function(){xhr("../language/api/base/security",{query:{auth:"role_ra"},handleAs:"json"}).then(function(){this.ra=true;dra.resolve();},function(){dra.resolve();});return dra.promise;};
		var fwa=function(){xhr("../language/api/base/security",{query:{auth:"role_wa"},handleAs:"json"}).then(function(){this.wa=true;dwa.resolve();},function(){dwa.resolve();});return dwa.promise;};
		...
		var fraroleauthority=function(){xhr("../language/api/base/security",{query:{auth:"rlau_ra"},handleAs:"json"}).then(function(){this.raroleauthority=true;draroleauthority.resolve();},function(){draroleauthority.resolve();});return draroleauthority.promise;};
		var friroleauthority=function(){xhr("../language/api/base/security",{query:{auth:"rlau_ri"},handleAs:"json"}).then(function(){this.riroleauthority=true;driroleauthority.resolve();},function(){driroleauthority.resolve();});return driroleauthority.promise;};
		d0.promise.then(frauserrole).then(function(){if(!rauserrole)return friuserrole();}).then(fraroleauthority).then(function(){if(!raroleauthority)return friroleauthority();}).then(fra).then(fwa).then(function(){d.resolve();});
		d0.resolve();
		return d;
	}
	...
	{	if(items.length>i)
		{	xhr.del("../language/api/role_check"+"/"+items[i],{handleAs:"json"}).then(function(data)
			{	_f_recCheckDelete(deferred,items,i+1);
			},function(data)
			{	var d=data.response.data,status=data.response.status;
				msgAreaDelete_messages.put({group:"delete",level:"error",msgId:items[i]},{msg:Message[status]+"："+grid.row(items[i]).data().rlname,targetNode:null});
				_f_recCheckDelete(deferred,items,i+1);
			});
		}
		else
