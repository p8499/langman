...
{	parent.f_setTitle();
	...
	grid_store=new JsonRest({target:"../language/api/chunkcriteria",idProperty:"ccid",sortParam:"orderBy"});
	grid_structure=[
		{id:"ccid",field:"ccid",name:"條件編碼",width:"64px"},
		{id:"cccpid",field:"cccpid",name:"組合編碼",width:"64px"},
		{id:"ccsi",field:"ccsi",name:"子項號",width:"64px"},
		{id:"cctk",field:"cctk",name:"採樣",width:"128px"},
		{id:"cctg",field:"cctg",name:"標記",width:"128px"}];
	...
	{	var deferred=new Deferred();
		xhr.post("../language/api/chunkcriteria_check",{data:domForm.toJson("addForm"),handleAs:"json",headers:{'Content-Type': 'application/json'}}).then(function(data)
	...
	{	var deferred=new Deferred();
		...
		all([grid.store.get(id),xhr("../language/api/base/lock",{query:{obj:"ckct",id:id},handleAs:"json"})]).then(function(data)
		{	update_ccid.set("value",data[0].ccid);
			update_cccpid.set("value",data[0].cccpid);
			update_ccsi.set("value",data[0].ccsi);
			update_cctk.set("value",data[0].cctk);
			update_cctg.set("value",data[0].cctg);
			updateDialog.show();
			deferred.resolve(data);
		},function(data)
	...
	{	var deferred=new Deferred();
		xhr("../language/api/base/unlock",{query:{obj:"ckct",id:update_ccid.get("value")},handleAs:"json"}).then(function(data)
		{	updateDialog.hide();
			deferred.resolve(data);
		},function(data)
		{	updateDialog.hide();
	...
	{	var deferred=new Deferred();
		xhr.put("../language/api/chunkcriteria_check"+"/"+update_ccid.get("value"),{data:domForm.toJson("updateForm"),handleAs:"json",headers:{'Content-Type': 'application/json'}}).then(function(data)
		{	deferred.resolve(data);
		},function(data)
	...
	{	var deferred=new Deferred();
		...
		all([grid.store.put(obj),xhr("../language/api/base/unlock",{query:{obj:"ckct",id:update_ccid.get("value")},handleAs:"json"})]).then(function(data)
		{	updateDialog.hide();
			deferred.resolve(data);
		},function(data)
		{	msgAreaUpdate_messages.removeAll({group:"update"});
	...
	{	this.ra=this.wa=false;
		...
		var fra=function(){xhr("../language/api/base/security",{query:{auth:"ckct_ra"},handleAs:"json"}).then(function(){this.ra=true;dra.resolve();},function(){dra.resolve();});return dra.promise;};
		var fwa=function(){xhr("../language/api/base/security",{query:{auth:"ckct_wa"},handleAs:"json"}).then(function(){this.wa=true;dwa.resolve();},function(){dwa.resolve();});return dwa.promise;};
		...
		d0.promise.then(frachunk).then(function(){if(!rachunk)return frichunk();}).then(fra).then(fwa).then(function(){d.resolve();});
		d0.resolve();
		return d;
	}
	...
	{	if(items.length>i)
		{	xhr.del("../language/api/chunkcriteria_check"+"/"+items[i],{handleAs:"json"}).then(function(data)
			{	_f_recCheckDelete(deferred,items,i+1);
			},function(data)
			{	var d=data.response.data,status=data.response.status;
				msgAreaDelete_messages.put({group:"delete",level:"error",msgId:items[i]},{msg:Message[status]+"："+grid.row(items[i]).data().cctk+" "+grid.row(items[i]).data().cctg,targetNode:null});
				_f_recCheckDelete(deferred,items,i+1);
			});
		}
		else
