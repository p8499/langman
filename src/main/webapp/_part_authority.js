...
{	parent.f_setTitle();
	...
	grid_store=new JsonRest({target:"../language/api/authority",idProperty:"auid",sortParam:"orderBy"});
	grid_structure=[
	...
	{	msgAreaAdd_messages.removeAll({group:"add"});
		...
		add_auid.set("value","");
		add_augrp.set("value","");
		add_auname.set("value","");
		addDialog.show();
	};
	this.f_checkAdd=function()
	{	var deferred=new Deferred();
		xhr.post("../language/api/authority_check"+"/"+add_auid.get("value"),{data:domForm.toJson("addForm"),handleAs:"json",headers:{'Content-Type': 'application/json'}}).then(function(data)
	...
	{	var deferred=new Deferred();
		...
		all([grid.store.get(id),xhr("../language/api/base/lock",{query:{obj:"auth",id:id},handleAs:"json"})]).then(function(data)
		{	update_auid.set("value",data[0].auid);
			update_augrp.set("value",data[0].augrp);
			update_auname.set("value",data[0].auname);
			updateDialog.show();
			deferred.resolve(data);
		},function(data)
	...
	{	var deferred=new Deferred();
		xhr("../language/api/base/unlock",{query:{obj:"auth",id:update_auid.get("value")},handleAs:"json"}).then(function(data)
		{	updateDialog.hide();
			deferred.resolve(data);
		},function(data)
		{	updateDialog.hide();
	...
	{	var deferred=new Deferred();
		xhr.put("../language/api/authority_check"+"/"+update_auid.get("value"),{data:domForm.toJson("updateForm"),handleAs:"json",headers:{'Content-Type': 'application/json'}}).then(function(data)
		{	deferred.resolve(data);
		},function(data)
	...
	{	var deferred=new Deferred();
		...
		all([grid.store.put(obj),xhr("../language/api/base/unlock",{query:{obj:"auth",id:update_auid.get("value")},handleAs:"json"})]).then(function(data)
		{	updateDialog.hide();
			deferred.resolve(data);
		},function(data)
		{	msgAreaUpdate_messages.removeAll({group:"update"});
	...
	{	this.ra=this.wa=false;
		...
		var fra=function(){xhr("../language/api/base/security",{query:{auth:"auth_ra"},handleAs:"json"}).then(function(){this.ra=true;dra.resolve();},function(){dra.resolve();});return dra.promise;};
		var fwa=function(){xhr("../language/api/base/security",{query:{auth:"auth_wa"},handleAs:"json"}).then(function(){this.wa=true;dwa.resolve();},function(){dwa.resolve();});return dwa.promise;};
		...
		d0.promise.then(fraroleauthority).then(function(){if(!raroleauthority)return friroleauthority();}).then(fra).then(fwa).then(function(){d.resolve();});
		d0.resolve();
		return d;
	}
	...
	{	if(items.length>i)
		{	xhr.del("../language/api/authority_check"+"/"+items[i],{handleAs:"json"}).then(function(data)
			{	_f_recCheckDelete(deferred,items,i+1);
			},function(data)
			{	var d=data.response.data,status=data.response.status;
				msgAreaDelete_messages.put({group:"delete",level:"error",msgId:items[i]},{msg:Message[status]+"："+grid.row(items[i]).data().auname,targetNode:null});
				_f_recCheckDelete(deferred,items,i+1);
			});
		}
		else
