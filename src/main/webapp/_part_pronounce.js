...
{	parent.f_setTitle();
	...
	grid_store=new JsonRest({target:"../language/api/pronounce",idProperty:"pnid",sortParam:"orderBy"});
	grid_structure=[
		{id:"pnid",field:"pnid",name:"字音編碼",width:"64px"},
		{id:"pnlsid",field:"pnlsid",name:"語言編碼",width:"128px"},
		{id:"pnct",field:"pnct",name:"字",width:"48px"},
		{id:"pnpi",field:"pnpi",name:"拼音",width:"128px"},
		{id:"pntn",field:"pntn",name:"聲調",width:"48px"},
		{id:"pnco",field:"pnco",name:"聲母",width:"64px"},
		{id:"pnvo",field:"pnvo",name:"韻母",width:"64px"},
		{id:"pncl",field:"pncl",name:"類別",width:"48px"},
		{id:"pnrm",field:"pnrm",name:"備註",width:"48px"}];
	grid_filterSetupQuery=function(expr)
	{	return {"filter":expr==null?null:json.stringify(expr)};
	};
	rel_language_pronounceGrid_store=new JsonRest({target:"../language/api/language_mask",idProperty:"lsid",sortParam:"orderBy"});
	rel_language_pronounceGrid_structure=[
	...
	rel_language_pronounceGrid_filterSetupQuery=function(expr)
	{	return {"filter":expr==null?null:json.stringify(expr)};
	};
	this.f_prepareAdd=function()
	{	msgAreaAdd_messages.removeAll({group:"add"});
		msgAreaAdd.refresh();
		add_pnlsid.set("value","");
		add_pnct.set("value","");
		add_pnpi.set("value","");
		add_pntn.set("value","");
		add_pnco.set("value","");
	...
	{	var deferred=new Deferred();
		xhr.post("../language/api/pronounce_check",{data:domForm.toJson("addForm"),handleAs:"json",headers:{'Content-Type': 'application/json'}}).then(function(data)
	...
	{	var deferred=new Deferred();
		...
		all([grid.store.get(id),xhr("../language/api/base/lock",{query:{obj:"pron",id:id},handleAs:"json"})]).then(function(data)
		{	update_pnid.set("value",data[0].pnid);
			update_pnlsid.set("value",data[0].pnlsid);
			update_pnct.set("value",data[0].pnct);
			update_pnpi.set("value",data[0].pnpi);
			update_pntn.set("value",data[0].pntn);
			update_pnco.set("value",data[0].pnco);
			update_pnvo.set("value",data[0].pnvo);
			update_pncl.set("value",data[0].pncl);
			update_pnrm.set("value",data[0].pnrm);
			updateDialog.show();
			deferred.resolve(data);
		},function(data)
	...
	{	var deferred=new Deferred();
		xhr("../language/api/base/unlock",{query:{obj:"pron",id:update_pnid.get("value")},handleAs:"json"}).then(function(data)
		{	updateDialog.hide();
			deferred.resolve(data);
		},function(data)
		{	updateDialog.hide();
	...
	{	var deferred=new Deferred();
		xhr.put("../language/api/pronounce_check"+"/"+update_pnid.get("value"),{data:domForm.toJson("updateForm"),handleAs:"json",headers:{'Content-Type': 'application/json'}}).then(function(data)
		{	deferred.resolve(data);
		},function(data)
	...
	{	var deferred=new Deferred();
		...
		all([grid.store.put(obj),xhr("../language/api/base/unlock",{query:{obj:"pron",id:update_pnid.get("value")},handleAs:"json"})]).then(function(data)
		{	updateDialog.hide();
			deferred.resolve(data);
		},function(data)
		{	msgAreaUpdate_messages.removeAll({group:"update"});
	...
	{	grid.connect(grid.select.row,"onSelected",function(row)
		...
		rel_language_pronounceGrid.connect(rel_language_pronounceGrid.select.row,"onSelected",function(row)
		{	f_assistEnd(rel_language_pronounceDialog,rel_language_pronounceGrid,row);
		});
	...
	{	this.ra=this.wa=false;
		...
		var fra=function(){xhr("../language/api/base/security",{query:{auth:"pron_ra"},handleAs:"json"}).then(function(){this.ra=true;dra.resolve();},function(){dra.resolve();});return dra.promise;};
		var fwa=function(){xhr("../language/api/base/security",{query:{auth:"pron_wa"},handleAs:"json"}).then(function(){this.wa=true;dwa.resolve();},function(){dwa.resolve();});return dwa.promise;};
	...
	{	if(items.length>i)
		{	xhr.del("../language/api/pronounce_check"+"/"+items[i],{handleAs:"json"}).then(function(data)
			{	_f_recCheckDelete(deferred,items,i+1);
			},function(data)
			{	var d=data.response.data,status=data.response.status;
				msgAreaDelete_messages.put({group:"delete",level:"error",msgId:items[i]},{msg:Message[status]+"："+grid.row(items[i]).data().pnct,targetNode:null});
				_f_recCheckDelete(deferred,items,i+1);
			});
		}
		else
